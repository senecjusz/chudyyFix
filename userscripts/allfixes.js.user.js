...
// === Bocra PDF (range selector + modal) ===
(function() {
    'use strict';

    // If your main script uses site-hash gating, you can wrap this block with that check.
    // Example (pseudo):
    // if (!isAllowedSite('skany-viewer')) return;

    const LEFT_COL_SELECTOR  = '.mt-col.mt-col-left img';
    const RIGHT_COL_SELECTOR = '.mt-col.mt-col-right img';
    const MAIN_IMG_SELECTOR  = '#documentImage';

    const BOCRA_ENDPOINT = 'https://bocra.shase.eu/api/images-to-ocr-pdf';

    let rangeMode = false;
    let rangeStartImg = null;
    let rangeEndImg = null;
    let masterUrls = null;
    let pdfButtonEl = null;

    function log() {
        console.log('[BocraPDF]', ...arguments);
    }
    ...
    function normalizeSrc(img) {
        if (!img) return '';
        let src = img.getAttribute('data-full') || img.getAttribute('data-src') || img.src;
        if (!src) return '';
        try {
    ...
    function getColumnImages() {
        const left = Array.from(document.querySelectorAll(LEFT_COL_SELECTOR));
        const right = Array.from(document.querySelectorAll(RIGHT_COL_SELECTOR));
        return left.concat(right);
    }

    function getAllSelectableImages() {
        const imgs = getColumnImages();
        const main = document.querySelector(MAIN_IMG_SELECTOR);
        if (main) imgs.push(main);
        return imgs;
    }

    function findHiddenSourceContainer() {
        ...
        if (best && bestCount > 0) {
            log('Hidden source container found with', bestCount, 'images');
            return best;
        }
        log('Hidden source container not found, will use column images for ordering');
        return null;
    }

    function getMasterUrls() {
        if (masterUrls) return masterUrls;

        const hidden = findHiddenSourceContainer();
        if (hidden) {
            masterUrls = Array.from(hidden.querySelectorAll('img')).map(normalizeSrc);
        } else {
            masterUrls = getColumnImages().map(normalizeSrc);
        }
        return masterUrls;
    }
    ...
    function buildDefaultFilenameFromUrls(urls) {
        if (!urls || !urls.length) {
            return buildOutputFilename();
        }
        const firstBase = baseNameFromUrl(urls[0]) || 'first';
        const lastBase  = baseNameFromUrl(urls[urls.length - 1]) || 'last';
        const combined  = firstBase + '-' + lastBase;

        let safeBase = combined.replace(/[^A-Za-z0-9._\- ]+/g, '_').trim();
        if (!safeBase) {
            safeBase = 'scans_ocr';
        }
        if (!/\.pdf$/i.test(safeBase)) {
            safeBase += '.pdf';
        }
        return safeBase;
    }
    ...
    function ensureStyles() {
        if (document.getElementById('bocra-pdf-styles')) return;
        const style = document.createElement('style');
        style.id = 'bocra-pdf-styles';
        style.textContent = `
      .bocra-pdf-selectable {
        cursor: crosshair !important;
        box-shadow: 0 0 0 2px rgba(0, 150, 255, 0.7);
      }
      .bocra-pdf-start {
        box-shadow: 0 0 0 3px rgba(0, 200, 0, 0.9) !important;
      }
      .bocra-pdf-end {
        box-shadow: 0 0 0 3px rgba(255, 0, 0, 0.9) !important;
      }
      .bocra-pdf-btn.bocra-pdf-active {
      ...
      .bocra-pdf-dialog-header {
        font-weight: bold;
        margin-bottom: 4px;
      }
      .bocra-pdf-dialog-body {
        display: flex;
        flex-direction: column;
        gap: 10px;
      }
      ...
      .bocra-pdf-preview-box {
        text-align: center;
      }
      .bocra-pdf-preview-box img {
        max-height: 300px;
        max-width: 100%;
      }
      .bocra-pdf-count {
      ...
      .bocra-pdf-dialog-footer {
        display: flex;
        justify-content: flex-end;
        gap: 10px;
        margin-top: 8px;
      }
      .bocra-pdf-dialog-footer button {
        padding: 6px 14px;
        font-size: 13px;
        cursor: pointer;
      }
    ...
    function showInfo(text) {
        ensureStyles();
        let info = document.getElementById('bocra-pdf-info');
        if (!info) {
            info = document.createElement('div');
            info.id = 'bocra-pdf-info';
    ...
    function hideInfo() {
        const info = document.getElementById('bocra-pdf-info');
        if (info) info.style.display = 'none';
    }

    function showOverlay(text) {
        ensureStyles();
        let o = document.getElementById('bocra-ocr-overlay');
        if (!o) {
            o = document.createElement('div');
            o.id = 'bocra-ocr-overlay';
            o.textContent = text || 'Processing in Bocra...';
    ...
    function hideOverlay() {
        const o = document.getElementById('bocra-ocr-overlay');
    ...
    function onThumbClick(e) {
        ...
        if (!rangeStartImg) {
            rangeStartImg = img;
            img.classList.add('bocra-pdf-start');
            showInfo('PDF mode: click first image, then last image. Press Esc to cancel.');
            return;
        }

        if (!rangeEndImg) {
            rangeEndImg = img;
            img.classList.add('bocra-pdf-end');
            finalizeRange();
        }
    }

    function attachRangeListeners(img) {
        if (img.dataset.bocraPdfBound === '1') return;
        img.addEventListener('click', onThumbClick, true);
        img.addEventListener('mousedown', swallowEvent, true);
        img.addEventListener('mouseup', swallowEvent, true);
        img.addEventListener('dblclick', swallowEvent, true);
        img.dataset.bocraPdfBound = '1';
    }

    function detachRangeListeners(img) {
        if (img.dataset.bocraPdfBound !== '1') return;
        img.removeEventListener('click', onThumbClick, true);
        img.removeEventListener('mousedown', swallowEvent, true);
        img.removeEventListener('mouseup', swallowEvent, true);
        img.removeEventListener('dblclick', swallowEvent, true);
        delete img.dataset.bocraPdfBound;
    }
    ...
    function enableRangeMode() {
        ...
        const imgs = getAllSelectableImages();
        imgs.forEach(img => {
            img.classList.add('bocra-pdf-selectable');
            attachRangeListeners(img);
        });

        if (pdfButtonEl) {
            pdfButtonEl.classList.add('bocra-pdf-active');
        }
    ...
    function disableRangeMode(showMessage) {
        rangeMode = false;
        const imgs = getAllSelectableImages();
        imgs.forEach(img => {
            img.classList.remove('bocra-pdf-selectable', 'bocra-pdf-start', 'bocra-pdf-end');
            detachRangeListeners(img);
        });
        if (pdfButtonEl) {
            pdfButtonEl.classList.remove('bocra-pdf-active');
        }
        if (showMessage) {
            showInfo('PDF mode disabled.');
    ...
    function buildRangeUrls(startImg, endImg) {
        const allUrls = getMasterUrls();
        const startUrl = normalizeSrc(startImg);
        const endUrl = normalizeSrc(endImg);

        let startIndex = allUrls.indexOf(startUrl);
        let endIndex = allUrls.indexOf(endUrl);

        if (startIndex === -1 || endIndex === -1) {
            log('Start or end not found in master list, falling back to column order');
            const colImgs = getColumnImages();
            const colUrls = colImgs.map(normalizeSrc);
            startIndex = colUrls.indexOf(startUrl);
            endIndex = colUrls.indexOf(endUrl);
            if (startIndex === -1 || endIndex === -1) {
                return [];
            }
            const from = Math.min(startIndex, endIndex);
            const to = Math.max(startIndex, endIndex);
            return colUrls.slice(from, to + 1);
        }

        const from = Math.min(startIndex, endIndex);
        const to = Math.max(startIndex, endIndex);
        return allUrls.slice(from, to + 1);
    }

    function finalizeRange() {
        if (!rangeStartImg || !rangeEndImg) return;

        const urls = buildRangeUrls(rangeStartImg, rangeEndImg);
        disableRangeMode(false);

        if (!urls.length) {
            showInfo('Could not determine selected range.');
            setTimeout(hideInfo, 2500);
            return;
        }

        log('Selected range URLs:', urls);
        openBocraConfirmModal(urls);
    }

    document.addEventListener('keydown', e => {
        if (e.key === 'Escape' && rangeMode) {
            disableRangeMode(true);
        }
    });
    ...
    function buildOutputFilename() {
        ...
        const ts = d.getFullYear().toString() +
            ...
            pad(d.getMinutes()) +
            pad(d.getSeconds());
        return 'scans_ocr_' + ts + '.pdf';
    }

    function bocraSendImages(imageUrls, outputFilename) {
        if (!imageUrls || !imageUrls.length) {
            alert('No images to send to Bocra.');
            return;
        }

        const safeName = outputFilename || buildOutputFilename();

        const payload = {
            images: imageUrls,
            do_ocr: true,
            output_filename: safeName
        };

        showOverlay('Processing in Bocra...');
        ...
        GM_xmlhttpRequest({
            ...
            onload: function(res) {
                ...
                if (res.status === 401) {
                    alert('To use this function, please log in to the Bocra app in this browser and try again.');
                    window.open('https://bocra.shase.eu/', '_blank');
                    return;
                }

                if (res.status < 200 || res.status >= 300) {
                    console.error('Bocra HTTP error', res.status, res.responseText);
                    alert('Bocra returned HTTP error: ' + res.status);
                    return;
                }
                ...
                try {
                    ...
                    console.error('Error handling Bocra response', e);
                    alert('Failed to save PDF returned by Bocra.');
                }
            },
            onerror: function(err) {
                hideOverlay();
                console.error('Bocra request error', err);
                alert('Could not connect to Bocra.');
            }
        });
    }
    ...
    function ensureModal() {
        ensureStyles();
        let modal = document.getElementById('bocra-pdf-modal');
        if (modal) return modal;

        modal = document.createElement('div');
        modal.id = 'bocra-pdf-modal';

        const dialog = document.createElement('div');
        dialog.className = 'bocra-pdf-dialog';

        const header = document.createElement('div');
        header.className = 'bocra-pdf-dialog-header';
        header.textContent = 'Bocra OCR – selected range';

        const body = document.createElement('div');
        body.className = 'bocra-pdf-dialog-body';

        const previews = document.createElement('div');
        previews.className = 'bocra-pdf-previews';

        const boxFirst = document.createElement('div');
        boxFirst.className = 'bocra-pdf-preview-box';
        const firstLabel = document.createElement('div');
        firstLabel.textContent = 'First scan';
        const firstImg = document.createElement('img');
        firstImg.id = 'bocra-pdf-preview-first';
        boxFirst.appendChild(firstLabel);
        boxFirst.appendChild(firstImg);

        const boxLast = document.createElement('div');
        boxLast.className = 'bocra-pdf-preview-box';
        const lastLabel = document.createElement('div');
        lastLabel.textContent = 'Last scan';
        const lastImg = document.createElement('img');
        lastImg.id = 'bocra-pdf-preview-last';
        boxLast.appendChild(lastLabel);
        boxLast.appendChild(lastImg);

        previews.appendChild(boxFirst);
        previews.appendChild(boxLast);

        const countInfo = document.createElement('div');
        countInfo.className = 'bocra-pdf-count';
        countInfo.innerHTML = 'Selected <span id="bocra-pdf-count">0</span> images.';

        const filenameBox = document.createElement('div');
        filenameBox.className = 'bocra-pdf-filename';
        const filenameLabel = document.createElement('div');
        filenameLabel.textContent = 'PDF file name:';
        const filenameInput = document.createElement('input');
        filenameInput.type = 'text';
        filenameInput.id = 'bocra-pdf-filename-input';
        filenameInput.placeholder = 'e.g. IMG_0123-IMG_0130.pdf';

        filenameBox.appendChild(filenameLabel);
        filenameBox.appendChild(filenameInput);

        body.appendChild(previews);
        body.appendChild(countInfo);
        body.appendChild(filenameBox);

        const footer = document.createElement('div');
        footer.className = 'bocra-pdf-dialog-footer';

        const btnConfirm = document.createElement('button');
        btnConfirm.id = 'bocra-pdf-confirm';
        btnConfirm.className = 'bocra-btn-primary';
        btnConfirm.textContent = 'Convert to PDF';

        const btnCancel = document.createElement('button');
        btnCancel.id = 'bocra-pdf-cancel';
        btnCancel.className = 'bocra-btn-secondary';
    ...
    function openBocraConfirmModal(urls) {
        const modal = ensureModal();
        const firstImg = modal.querySelector('#bocra-pdf-preview-first');
        const lastImg = modal.querySelector('#bocra-pdf-preview-last');
        const countSpan = modal.querySelector('#bocra-pdf-count');
        const btnConfirm = modal.querySelector('#bocra-pdf-confirm');
        const btnCancel = modal.querySelector('#bocra-pdf-cancel');
        const filenameInput = modal.querySelector('#bocra-pdf-filename-input');

        firstImg.src = urls[0];
        lastImg.src = urls[urls.length - 1];
        countSpan.textContent = urls.length.toString();
        filenameInput.value = buildDefaultFilenameFromUrls(urls);
        ...
        const newConfirm = modal.querySelector('#bocra-pdf-confirm');
        const newCancel = modal.querySelector('#bocra-pdf-cancel');

        newConfirm.addEventListener('click', () => {
            const raw = filenameInput.value.trim();
            const fallback = buildDefaultFilenameFromUrls(urls);
            const outName = sanitizeFilename(raw, fallback);
            close();
            bocraSendImages(urls, outName);
        });
    ...
    // ---------- PDF button next to Links ----------

    function findLinksButton() {
        const candidates = Array.from(document.querySelectorAll('button, a'));
        return candidates.find(el => el.textContent && el.textContent.trim() === 'Links');
    }

    function addPdfButton() {
        if (document.querySelector('.bocra-pdf-btn')) return;
        const linksBtn = findLinksButton();
        if (!linksBtn || !linksBtn.parentNode) return;

        const pdfBtn = document.createElement(linksBtn.tagName.toLowerCase());
        pdfBtn.className = linksBtn.className
            ? (linksBtn.className + ' bocra-pdf-btn')
            : 'bocra-pdf-btn';

        if (pdfBtn.tagName.toLowerCase() === 'button') {
            pdfBtn.type = 'button';
        }
        ...
        linksBtn.parentNode.insertBefore(pdfBtn, linksBtn.nextSibling);
        pdfButtonEl = pdfBtn;
        log('PDF button added');
    }
    ...
    const interval = setInterval(() => {
        // this will only do anything on pages where a "Links" button exists
        if (findLinksButton()) {
